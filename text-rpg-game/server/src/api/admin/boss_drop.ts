...
const bossDropGetter = {
  get: async (id: number) => {
    const d = await dataStorageService.getByCondition(Collections.BOSS_DROP, { id });
    if (!d) return null;
    const [item, boss] = await Promise.all([
      dataStorageService.getByCondition(Collections.ITEM, { id: d.item_id }),
      dataStorageService.getByCondition(Collections.BOSS, { id: d.boss_id })
    ]);
    return { ...d, item_name: item?.name, boss_name: boss?.name };
  }
};
const bossDropDeleter = { delete: (id: number) => dataStorageService.delete(Collections.BOSS_DROP, id) };
...
router.get('/', adminHandler(async (req, res) => {
  ...
  const list = await dataStorageService.list(Collections.BOSS_DROP, filter);
  const [items, bosses] = await Promise.all([
    dataStorageService.list(Collections.ITEM, undefined),
    dataStorageService.list(Collections.BOSS, undefined)
  ]);
...
router.post('/', adminHandler(async (req, res) => {
  ...
  const [boss, item] = await Promise.all([
    dataStorageService.getByCondition(Collections.BOSS, { id: Number(boss_id) }),
    dataStorageService.getByCondition(Collections.ITEM, { id: Number(item_id) })
  ]);
  ...
  const insertId = await dataStorageService.insert(Collections.BOSS_DROP, data);
...
router.put('/:id', adminHandler(async (req, res) => {
  ...
  if (boss_id != null) {
    const boss = await dataStorageService.getByCondition(Collections.BOSS, { id: Number(boss_id) });
    if (!boss) return fail(res, ErrorCode.NOT_FOUND, 'Boss 不存在');
  ...
  const ok = await dataStorageService.update(Collections.BOSS_DROP, id, updateData);
