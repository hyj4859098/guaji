/**
 * GM Boss 掉落管理 API
 * boss_drop 表：boss_id, item_id, quantity, probability
 */
...
router.get('/', async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    const bosses = await dataStorageService.list('boss', undefined);
    ...
    logger.error('获取 Boss 掉落列表失败:', error);
    next(error);
  }
});
...
router.get('/:id', async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    const boss = await dataStorageService.getByCondition('boss', { id: d.boss_id });
    success(res, { ...d, item_name: item?.name, boss_name: boss?.name });
  } catch (error) {
    logger.error('获取 Boss 掉落失败:', error);
    next(error);
  }
});
...
router.post('/', async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    logger.error('新增 Boss 掉落失败:', error);
    next(error);
  }
});
...
router.put('/:id', async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    logger.error('更新 Boss 掉落失败:', error);
    next(error);
  }
});
...
router.delete('/:id', async (req: any, res: Response, next: NextFunction) => {
  try {
    const id = parseInt(req.params.id);
    if (isNaN(id)) return fail(res, ErrorCode.INVALID_PARAMS, '无效的ID');
    const ok = await dataStorageService.delete('boss_drop', id);
    if (ok) success(res, { message: '删除成功' });
    else fail(res, ErrorCode.NOT_FOUND, '删除失败');
  } catch (error) {
    logger.error('删除 Boss 掉落失败:', error);
    next(error);
  }
});
