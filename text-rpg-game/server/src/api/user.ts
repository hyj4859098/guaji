...
router.post('/login', authLimiter, asyncHandler(async (req: Request, res: Response) => {
  ...
  if (hashed) {
    ...
    await dataStorageService.updateByFilter(Collections.USER, updateFilter, { password: hashed });
...
router.post('/register', authLimiter, asyncHandler(async (req: Request, res: Response) => {
  ...
  const existing = await dataStorageService.getByCondition(Collections.USER, { username });
  ...
  const userId = await dataStorageService.insert(Collections.USER, {
    username,
    password: hashed,
