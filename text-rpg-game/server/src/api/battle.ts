...
router.post('/start', auth, validate(battleStartBody), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    logger.error(`战斗失败 - uid: ${req.uid}, 错误: ${error}`);
    next(error);
  }
});
...
router.post('/auto', auth, validate(battleAutoBody), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    logger.error(`自动战斗失败 - uid: ${req.uid}, 错误: ${error}`);
    next(error);
  }
});

router.post('/resume', auth, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    logger.error(`恢复自动战斗失败 - uid: ${req.uid}, 错误: ${error}`);
    next(error);
  }
});
...
router.post('/stop', auth, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    logger.error(`停止战斗失败 - uid: ${req.uid}, 错误: ${error}`);
    next(error);
  }
});

router.get('/status', auth, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    logger.error(`获取战斗状态失败 - uid: ${req.uid}, 错误: ${error}`);
    next(error);
  }
});
