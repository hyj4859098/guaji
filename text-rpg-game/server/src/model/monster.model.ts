...
export class MonsterModel implements IBaseModel<Monster> {
  async get(id: Id, ctx?: any): Promise<Monster | null> {
    return await dataStorageService.getById(Collections.MONSTER, id, ctx);
  }

  async list(_ctx?: any): Promise<Monster[]> {
    // MongoDB查询，按level字段升序排序
    const collection = getCollection(Collections.MONSTER);
  ...
  async insert(data: any, ctx?: any): Promise<Id> {
    return await dataStorageService.insert(Collections.MONSTER, data, ctx);
  }

  async update(id: Id, data: Partial<Monster>, ctx?: any): Promise<boolean> {
    return await dataStorageService.update(Collections.MONSTER, id, data, ctx);
  }

  async delete(id: Id, ctx?: any): Promise<boolean> {
    return await dataStorageService.delete(Collections.MONSTER, id, ctx);
  }
}
