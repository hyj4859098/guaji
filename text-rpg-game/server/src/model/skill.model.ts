...
export class SkillModel implements IBaseModel<Skill> {
  async get(id: Id, ctx?: any): Promise<Skill | null> {
    return await dataStorageService.getById('skill', id, ctx);
  }

  async getBySkillId(skillId: number, ctx?: any): Promise<Skill | null> {
    return await dataStorageService.getById('skill', skillId, ctx);
  }

  async getByBookId(bookId: number, ctx?: any): Promise<Skill | null> {
    return await dataStorageService.getByCondition('skill', { book_id: bookId }, ctx);
  }

  async list(ctx?: any): Promise<Skill[]> {
    return await dataStorageService.list('skill', undefined, ctx);
  }

  async insert(data: any, ctx?: any): Promise<Id> {
    return await dataStorageService.insert('skill', data, ctx);
  }

  async update(id: Id, data: Partial<Skill>, ctx?: any): Promise<boolean> {
    return await dataStorageService.update('skill', id, data, ctx);
  }

  async delete(id: Id, ctx?: any): Promise<boolean> {
    return await dataStorageService.delete('skill', id, ctx);
  }
}

export class PlayerSkillModel implements IBaseModel<PlayerSkill> {
  async get(id: Id, ctx?: any): Promise<PlayerSkill | null> {
    return await dataStorageService.getById('player_skill', id, ctx);
  }

  async getByUidAndSkillId(uid: Uid, skillId: number, ctx?: any): Promise<PlayerSkill | null> {
    let doc = await dataStorageService.getByCondition('player_skill', { uid, skill_id: skillId }, ctx);
    if (!doc && uid != null) {
  ...
  async listByUid(uid: Uid, ctx?: any): Promise<PlayerSkill[]> {
    ...
    if (list.length === 0 && uid != null) {
      if (typeof uid === 'number') {
        list = await dataStorageService.list('player_skill', { uid: String(uid) }, ctx);
  ...
  async listEquippedByUid(uid: Uid, type: number, ctx?: any): Promise<PlayerSkill[]> {
    ...
    let playerSkills = await dataStorageService.list('player_skill', { uid, is_equipped: 1 }, ctx);
    if (playerSkills.length === 0 && uid != null) {
      const altUid = typeof uid === 'number' ? String(uid) : Number(uid);
      if (!Number.isNaN(altUid as number)) {
        playerSkills = await dataStorageService.list('player_skill', { uid: altUid, is_equipped: 1 }, ctx);
      }
    }
    if (playerSkills.length === 0) return [];

    const skillIds = playerSkills.map(ps => ps.skill_id);
    const skills = await dataStorageService.list('skill', { id: { $in: skillIds }, type }, ctx);
  ...
  async insert(data: any, ctx?: any): Promise<Id> {
    ...
    return await dataStorageService.insert('player_skill', insertData, ctx);
  }
  ...
  async deleteByUid(uid: Uid, ctx?: any): Promise<void> {
    await dataStorageService.deleteMany('player_skill', { uid }, ctx);
  }
}
